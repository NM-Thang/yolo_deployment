...
class VideoTracker:
    def __init__(self):
        ...
        self.output_path = args.output_path + \
            self.video_path.split("/")[-1] if args.output_path else ""
    ...
    def run(self):
        ...
        while cap.isOpened():
            ...
            if len(frames_buffer) == self.batch_size or (not ret and len(frames_buffer) > 0):
                ...
                batch_input = batch_preprocessing(frames_buffer)
                # t1 = time.perf_counter()
                batch_outputs = self.triton_client.run(batch_input=batch_input)
                # t2 = time.perf_counter()

                # (W, H) for postprocessing
                input_size = (batch_input.shape[3], batch_input.shape[2])
                for i in range(len(frames_buffer)):
                    ...
                    for obj in tracked_objects:
                        ...
                        cv2.rectangle(current_frame, (x1, y1),
                                      (x2, y2), (0, 255, 0), 2)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Triton Batch Tracking Example")
    parser.add_argument("--video-path", type=str,
                        default="data/videos/people-detection.mp4", help="Path to input video")
    parser.add_argument("--batch-size", type=int, default=8,
                        help="Batch size for inference")
    parser.add_argument("--output-path", type=str, default="data/videos/results/",
