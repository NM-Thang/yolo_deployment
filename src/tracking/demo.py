...
class VideoPipeline:
    ...
    def reader_thread(self) -> None:
        ...
        while self.is_running:
            ...
            if not ret:
                break
            frame_data = {"image": frame, "fid": int(cap.get(cv2.CAP_PROP_POS_FRAMES))}
    ...
    def worker_thread(self) -> None:
        ...
        while self.is_running:
            try:
                ...
                out = draw_frame(frame, tracked_objects, self.ZoneEventDetector.zone.coordinates)
                if not self.result_queue.full():
                    self.result_queue.put({"frame": out, "event": event, "fid": fid})
    ...
    def restreamer_thread(self) -> None:
        ...
        while self.is_running:
            ...
            while self.result_queue.qsize() < int(self.fps * 5):
                time.sleep(0.01)
...
def demo_multithread() -> None:
    ...
    zone = Zone(name="Entrance", coordinates=[(250, 150), (450, 150), (450, 350), (250, 350)])
    zone_event_detector = ZoneEventDetector(zone=zone, detector_adapter=adapter, semaphore_limit=2)
...
async def main():
    ...
    adapter = DetectorAdapter(triton_client)
    zone = Zone(name="Entrance", coordinates=[(250, 150), (450, 150), (450, 350), (250, 350)])
    event_detector = ZoneEventDetector(zone=zone, detector_adapter=adapter, semaphore_limit=3)
    ...
    while cap.isOpened():
        ...
        results = await event_detector.push_frame_async(frame, fid=int(cap.get(cv2.CAP_PROP_POS_FRAMES)))
