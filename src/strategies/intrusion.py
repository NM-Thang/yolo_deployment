...
class IntrusionDetection(AIStrategy):
    ...
    def process_frame(self, frame, *args, **kwargs):
        ...
        # print(f"Raw detections: {detections}")  # Debug statement to check raw detections
        ...
        for obj in tracked_objects:
            ...
            bbox = (x1, y1, x2, y2)
            dets_results.append({"bbox": bbox, "track_id": track_id, "label": "person"})
            ...
            if is_in_zone and track_id not in self.objects_in_zone:
                ...
                event.append(f"{time.strftime('%Y-%m-%d %H:%M:%S')}: person {track_id} entered the zone.")
            elif not is_in_zone and track_id in self.objects_in_zone:
                self.objects_in_zone.discard(track_id)
                event.append(f"{time.strftime('%Y-%m-%d %H:%M:%S')}: person {track_id} left the zone.")
        
        # Debug statements to check the flow of events and detections
        print(f"len detections: {len(detections)}")  # Debug statement to check number of detections
        print(f"len dets_results: {len(dets_results)}")  # Debug statement to check number of detections results
        print(f"id in objects_in_zone: {self.objects_in_zone}")  # Debug statement to check current tracked IDs in zone
